...
const Calculator = {
    //this is dispkays 0 on the screen
    Display_Value: '0',
...
//this modifies values each time a button is click
function Input_Digit(digit) {
    const { Display_Value, Wait_Second_Operand } = Calculator;
    // we are checking to see if Wait_Second_Operand is true and set
    // Display_Value to the ket that was clicked.
...
function Input_Decimal(dot) {
    //this ensures that accidental clikcing of the point
    // doesnt cause bugs in your opertaion
    if (Calculator.Wait_Second_Operand === true) return;
    if (!Calculator.Display_Value.includes(dot)) {
        //we are sayijng that if the Display_Value does not contain a decimal point
...
function Handle_Operator(Next_Operator) {
    ...
    // this functoin updates the screen with the contents of Display_Value
